...
Coq's `.PTree.theory.d` supplies 1639 direct local Require edges covering 195 ordinary modules out of 196 maintained modules. External libraries are excluded. Transitive clients include re-export paths; an import does not prove use of each declaration.
...
Root closure reaches 95 modules; 100 lie outside it. Unreachable modules can still be meaningful regressions or independent measure theorems. All remain built by Dune; no deletion follows from this classification.
...
- `PTree.Regression.Semantics.AtomicInterp` (0 direct local clients)
...
49 modules have no direct local client. This includes exported roots and executable/negative regression leaves, not just potential dead code.
...
- `PTree.Regression.Semantics.AtomicInterp` — built leaf
