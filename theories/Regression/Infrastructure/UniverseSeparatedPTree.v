...
(** The intended backend typechecks at this separated signature.  This
    definition is a compile-time regression test for this alternate design. *)
...
(** A two-level signature removes that cycle: [MN] interprets source-level
    ...
    This hypothetical alternate model must connect them with a promotion/bind
...
(** The abstract two-level shape typechecks, but today's
    ...
    be instantiated independently at both levels. Thus this alternate model
...
(** The decisive negative regression: that sealed target universe remains
    ...
    Bernoulli node into a recursive frontier carrier. This direct model
