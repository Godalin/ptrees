...
Require Import PTree.Prob.Backend.SubEnum.FreeOmega.UpperExpectation PTree.Prob.Backend.SubEnum.FreeOmega.UpperCoupling PTree.Prob.Backend.SubEnum.FreeOmega.UpperContinuity PTree.Prob.Backend.SubEnum.FreeOmega.UpperRelational.
...
Local Notation test := (bounded_test (R := F)).
...
Proof.
  intros [Hl Hr] HT. split; eapply free_omega_upper_rel_mono; eauto.
Qed.

Lemma upper_birel_ext {A B} (T : A -> B -> Prop) mu nu mu' nu' :
...
Lemma upper_birel_comp {A B C} (T : A -> B -> Prop) (U : B -> C -> Prop)
...
Proof.
  ...
  - eapply free_omega_upper_rel_mono; [eapply free_omega_upper_rel_comp; eassumption|exact HV].
  - eapply free_omega_upper_rel_mono; [eapply free_omega_upper_rel_comp; eassumption|].
    intros z x [y [Hy Hx]]. apply HV. exists y. by split.
Qed.

Lemma upper_birel_bind {A B C D} (T : A -> B -> Prop) (U : C -> D -> Prop)
    mu nu (k : A -> FreeOmega SubEnum C) (h : B -> FreeOmega SubEnum D) :
...
Proof.
  ...
  - eapply free_omega_upper_rel_bind; [exact Hl|]. intros x y Hxy. exact (proj1 (Hk x y Hxy)).
  - eapply free_omega_upper_rel_bind; [exact Hr|]. intros y x Hxy. exact (proj2 (Hk x y Hxy)).
Qed.

Lemma upper_birel_sample {A B C D} (T : A -> B -> Prop) (U : C -> D -> Prop)
    mu nu (k : A -> FreeOmega SubEnum C) (h : B -> FreeOmega SubEnum D) :
  sem_lift T mu nu -> (forall x y, T x y -> related U (k x) (h y)) ->
  related U (FOSample mu k) (FOSample nu h).
Proof.
  intros Hmu Hk. split.
  - eapply free_omega_upper_rel_sample; [exact Hmu|]. intros x y Hxy. exact (proj1 (Hk x y Hxy)).
  - eapply free_omega_upper_rel_sample; [apply sem_lift_sym; exact Hmu|].
    intros y x Hxy. exact (proj2 (Hk x y Hxy)).
Qed.

Lemma upper_birel_lub {A B} (T : A -> B -> Prop) c d :
  (forall n, related T (c n) (d n)) -> related T (FOLub c) (FOLub d).
Proof.
  intro H. split; apply free_omega_upper_rel_lub; intro n;
    [exact (proj1 (H n))|exact (proj2 (H n))].
Qed.

Lemma upper_birel_sample_ae {A B C} (T : A -> B -> Prop) (mu : SubEnum C)
    (Good : C -> Prop) (k : C -> FreeOmega SubEnum A) (h : C -> FreeOmega SubEnum B) :
...
Lemma upper_sample_product {A X Y} (mu : SubEnum X) (nu : SubEnum Y)
    (k : X -> Y -> FreeOmega SubEnum A) f :
  upper (FOSample (semantic_product mu nu) (fun p => k (fst p) (snd p))) f =
  upper (FOSample mu (fun x => FOSample nu (k x))) f.
Proof.
  change (enum_real_expect
    (fun p => upper (k (fst p) (snd p)) f)
    (Enum.bind_Enum (subenum_raw mu) (fun x =>
      Enum.bind_Enum (subenum_raw nu) (fun y => Enum.ret_Enum (x,y)))) =
    enum_real_expect (fun x => enum_real_expect (fun y => upper (k x y) f)
      (subenum_raw nu)) (subenum_raw mu)).
  rewrite enum_real_expect_bind. f_equal. apply functional_extensionality=> x.
  rewrite enum_real_expect_bind. f_equal. apply functional_extensionality=> y.
  change (ratr (1 : rat) * upper (k x y) f + 0 = upper (k x y) f).
  by rewrite rmorph1 mul1r addr0.
Qed.

Lemma upper_zero_prefix {A} (c : nat -> FreeOmega SubEnum A) f :
  test f ->
...
Theorem free_omega_qlift_upper_birel {A B} (T : A -> B -> Prop) mu nu :
...
Proof.
  ...
  - split; intros f g Hf Hg Hfg.
    + exact (free_omega_structural_upper H Hg Hfg).
    + exact (free_omega_structural_upper (free_omega_lift_sym H) Hg Hfg).
  - split.
    + eapply free_omega_observes_upper_rel; eassumption.
    + eapply free_omega_observes_upper_rel; [exact H0|exact H|apply sem_lift_sym; exact H1|].
      intros y x Hxy. exact (H2 x y Hxy).
  - destruct IHHq as [Hl Hr]. split.
    + eapply free_omega_upper_rel_restrict; eassumption.
    + eapply free_omega_upper_rel_restrict; [exact Hr|exact H0|exact H|].
      intros y x [Hxy [Hy Hx]]. apply H1. by repeat split.
  - eapply upper_birel_mono; eassumption.
  - destruct IHHq as [Hl Hr]. by split.
  - eapply upper_birel_comp; eassumption.
  - eapply upper_birel_bind; eassumption.
  - eapply upper_birel_sample; eassumption.
  - eapply upper_birel_ext with (mu' := k x) (nu' := nu); [|reflexivity|exact IHHq].
    intros f Hf. change (ratr (1 : rat) * upper (k x) f + 0 = upper (k x) f).
    by rewrite rmorph1 mul1r addr0.
  - eapply upper_birel_ext with
        (mu' := FOSample (sem_bind mu h) k) (nu' := FOSample (sem_bind mu h) l).
    + intros f Hf. apply free_omega_sample_bind_upper.
    + reflexivity.
    + eapply upper_birel_sample with (T := eq).
  ...
  - eapply upper_birel_ext with (mu' := FOLub c) (nu' := FOLub d).
  ...
  - split; intros f g Hf Hg Hfg; cbn [free_omega_upper];
      rewrite enum_real_expect_zero; exact: lexx.
  - eapply upper_birel_ext with (mu' := mu) (nu' := nu); [reflexivity| |exact IHHq].
  ...
  - destruct H1 as [Hl Hr]. split; intros f g Hf Hg Hfg;
      eapply free_omega_cofinal_upper_le; eassumption.
Qed.

Corollary free_omega_qlift_upper {A B} (T : A -> B -> Prop) mu nu
    (f : A -> F) (g : B -> F) :
  @free_omega_qlift SubEnum SubEnum_SemanticMeasure SubEnum_SemanticOmega
    A B T mu nu ->
  test f -> test g -> (forall x y, T x y -> f x <= g y) ->
  upper mu f <= upper nu g.
Proof. intro H. exact (proj1 (free_omega_qlift_upper_birel H) f g). Qed.

Corollary free_omega_qlift_eq_upper {A} (mu nu : FreeOmega SubEnum A) f :
  @free_omega_qlift SubEnum SubEnum_SemanticMeasure SubEnum_SemanticOmega
    A A eq mu nu -> test f -> upper mu f = upper nu f.
Proof.
  intros H Hf. destruct (free_omega_qlift_upper_birel H) as [Hl Hr].
...
Proof.
  ...
  have Hone : forall X : Type, test (fun _ : X => (1 : F)).
