...
From PTree.Prob.Backend.SubEnum.FreeOmega Require Import UpperExpectation UpperCoupling.
...
Definition free_omega_admissible {A} (t : FreeOmega SubEnum A) : Prop :=
  OmegaValLaws (free_omega_upper (R := R) t).

Definition free_omega_domain {A} (t : FreeOmega SubEnum A)
...
Definition free_omega_domain_denotes {A} (t : FreeOmega SubEnum A)
    (L : OmegaVal R A) : Prop :=
  forall f, oval_test f -> free_omega_upper t f = oval_eval L f.

Theorem free_omega_admissible_iff_denotes {A} (t : FreeOmega SubEnum A) :
...
Lemma free_omega_admissible_ext {A} (t u : FreeOmega SubEnum A) :
...
Lemma admissible_zero {A} : free_omega_admissible (@FOZero SubEnum A).
Proof. exact (oval_laws (@oval_bottom R A)). Qed.

Lemma admissible_sample {A X} (mu : SubEnum X) (k : X -> FreeOmega SubEnum A) :
  (forall x, free_omega_admissible (k x)) ->
  free_omega_admissible (FOSample mu k).
Proof.
  intro H; exact (oval_laws (oval_bind (subenum_domain R mu)
    (fun x => free_omega_domain (H x)))).
Qed.

Definition free_omega_domain_increasing {A} (c : nat -> FreeOmega SubEnum A) :=
  forall n (f : A -> R), oval_test f ->
    free_omega_upper (c n) f <= free_omega_upper (c (S n)) f.

Lemma admissible_lub {A} (c : nat -> FreeOmega SubEnum A)
...
Lemma admissible_lub_approx {A} (c : nat -> FreeOmega SubEnum A) :
...
Lemma admissible_bind {A B} (t : FreeOmega SubEnum A)
    (k : A -> FreeOmega SubEnum B) :
...
Definition admissible_support_kernel {A} (t : FreeOmega SubEnum A) :
    FreeOmega SubEnum A :=
  if pselect (free_omega_admissible t) then t else FOZero.

Lemma admissible_support_kernel_valid {A} (t : FreeOmega SubEnum A) :
...
Lemma admissible_support_kernel_eq {A} (t : FreeOmega SubEnum A) :
...
Theorem admissible_bind_ae {A B} (t : FreeOmega SubEnum A)
    (k : A -> FreeOmega SubEnum B) :
  free_omega_admissible t ->
...
Theorem admissible_sample_ae {A X} (mu : SubEnum X)
    (k : X -> FreeOmega SubEnum A) :
