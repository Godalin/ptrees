(** Role: SubEnum specialization of native-parametric external validation.
...
From PTree.Prob.Backend.SubEnum.FreeOmega Require Import UpperExpectation Admissibility.
...
Section SubEnumValidation.
Variable R : realType.
Local Notation native := (fun X => @subenum_domain R X).

Theorem subenum_model_upper {A} (t : FreeOmega SubEnum A) f :
  free_omega_model_upper native t f = free_omega_upper t f.
Proof.
  reflexivity.
Qed.

Theorem subenum_modelable_iff_admissible {A} (t : FreeOmega SubEnum A) :
  free_omega_modelable native t <-> free_omega_admissible R t.
Proof.
  split; intro H; eapply oval_laws_ext; [exact H| |exact H|];
    intros f Hf; [symmetry|]; exact: subenum_model_upper.
Qed.

Theorem subenum_model_denotes_iff {A} (t : FreeOmega SubEnum A) L :
  free_omega_model_denotes native t L <-> free_omega_domain_denotes t L.
Proof.
  split; intros H f Hf.
  - rewrite -(subenum_model_upper t f); exact (H f Hf).
  - rewrite subenum_model_upper; exact (H f Hf).
Qed.

Theorem subenum_generic_domain_agrees {A} (t : FreeOmega SubEnum A)
    (H : free_omega_modelable native t) (Hds : free_omega_admissible R t) :
  oval_eq (free_omega_model H) (free_omega_domain Hds).
Proof. intros f Hf; exact: subenum_model_upper. Qed.

Theorem subenum_native_model_ae {X} (mu : SubEnum X) P :
  sem_ae mu P -> oval_ae (subenum_domain R mu) P.
Proof.
  intros Ha f g Hf Hg He; apply/eqP; rewrite eq_le; apply/andP; split;
    apply enum_real_expect_ae_mono; intros p x Hin Hnz.
  - rewrite (He x (Ha p x Hin Hnz)); exact: lexx.
  - rewrite (He x (Ha p x Hin Hnz)); exact: lexx.
Qed.

Theorem subenum_native_model_lift {X Y} (S : X -> Y -> Prop)
    (mu : SubEnum X) (nu : SubEnum Y) f g :
  sem_lift S mu nu -> oval_test f -> oval_test g ->
  (forall x y, S x y -> f x <= g y) ->
  oval_eval (subenum_domain R mu) f <= oval_eval (subenum_domain R nu) g.
Proof. intros H _ _ Hfg; exact (subenum_sem_lift_test_sound H Hfg). Qed.
End SubEnumValidation.
