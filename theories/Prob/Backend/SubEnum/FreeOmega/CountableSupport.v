...
From PTree.Prob.Backend.SubEnum.FreeOmega Require Import Admissibility UpperCoupling.
From PTree.Prob.Domain Require Import Expectation Countable.
Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.

Fixpoint free_omega_enumerate {A} (t : FreeOmega SubEnum A) (n : nat) : option A :=
  match t with
  | FORet x => Some x
  | FOZero => None
  | @FOSample _ _ X mu k =>
      ...
      | Some (i,j) =>
          match List.nth_error (subenum_raw mu) i with
...
Theorem free_omega_enumerate_covers {A} (t : FreeOmega SubEnum A) :
  free_omega_ae (oval_enumerated (free_omega_enumerate t)) t.
Proof.
  induction t as [x| |X mu k IH|c IH].
  - apply FOAERet; exists O; reflexivity.
  - apply FOAEZero.
  - apply FOAESample with (Good := fun x => exists w, List.In (w,x) (subenum_raw mu)).
...
Theorem free_omega_domain_enumerated {A} (t : FreeOmega SubEnum A)
...
Theorem free_omega_domain_countable {A} (t : FreeOmega SubEnum A)
...
Theorem free_omega_domain_countable_representation {A} (t : FreeOmega SubEnum A)
    (H : free_omega_admissible R t) :
