...
Require Import PTree.Prob.Backend.Common.RatSubTypes PTree.Prob.Backend.Enum.Representation PTree.Prob.Backend.Enum.Map PTree.Prob.Backend.Enum.Bind PTree.Prob.Backend.Enum.Iteration PTree.Prob.Backend.Enum.Support PTree.Prob.Backend.Enum.SemanticCoupling.
...
Require Import PTree.Prob.Backend.SubEnum.FreeOmega.UpperExpectation PTree.Prob.Backend.SubEnum.FreeOmega.UpperCoupling PTree.Prob.Backend.SubEnum.FreeOmega.UpperContinuity.

Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.
Import Enum PTree.Prob.Backend.Enum.Map RatSubTypes GRing.Theory Num.Theory Order.Theory.
...
Lemma enum_real_expect_atom_le {A : eqType} (f : A -> R) mu nu :
  (forall x, 0 <= f x) ->
...
Proof.
  ...
  - apply enum_real_expect_nonnegative. exact Hf.
  - rewrite (enum_real_expect_filter_split f ((p,a)::tail) a).
    rewrite (enum_real_expect_filter_split f nu a). apply lerD.
...
Lemma enum_indicator_atom {A : eqType} (mu : Enum A) a :
  enum_expect (fun x => if x == a then 1 else 0) mu = Qval (acc_mass a mu).
Proof.
  induction mu as [|[p x] tail IH]; cbn [enum_expect]; [reflexivity|].
...
Theorem enum_real_expect_atomic_lub {A : eqType} (out : Enum A)
    (chain : nat -> Enum A) (f : A -> R) :
  (forall x, 0 <= f x) ->
...
Proof.
  ...
  { intro n. apply enum_real_expect_atom_le; [exact Hf|]. intro x. exact (Hbound x n). }
  destruct target as [|[p a] tail].
  - apply/eqP. rewrite eq_le. apply/andP. split.
    + exact (countable_upper_le Hexpect).
    + cbn [enum_real_expect]. eapply le_trans.
      * exact (enum_real_expect_nonnegative (chain 0%nat) Hf).
      * exact (@countable_upper_ge R (fun n => expect f (chain n)) 0 0%nat Hexpect).
  - pose (rest := fun mu : Enum A => [seq h <- mu | snd h != a]).
    ...
    { apply functional_extensionality=> n.
      rewrite (enum_real_expect_filter_split f (chain n) a) mulrC. reflexivity. }
    rewrite Hsplit.
    ...
    + rewrite (@countable_upper_scale R _ (f a) (ratr (Qval (acc_mass a ((p,a)::tail))))
      ...
      rewrite (enum_real_expect_filter_split f ((p,a)::tail) a) mulrC. reflexivity.
    + intro n. exact (ler_wpM2l (Hf a) (Hinc a n)).
    + intro n. apply enum_real_expect_atom_le; [exact Hf|]. intro x.
...
Lemma enum_monotone_converges_upper_eqtype {A : eqType}
...
Proof.
  ...
  { intros x n. rewrite -!enum_indicator_atom.
  ...
  { intros x eps Heps. destruct (Hlim (fun y => y == x) eps Heps) as [N HN].
    exists N. intros n Hn. rewrite -!enum_indicator_atom. exact (HN n Hn). }
  apply enum_real_expect_atomic_lub; [exact Hf| | |].
...
Theorem enum_monotone_converges_upper {A : Type}
    (chain : nat -> Enum A) out (f : A -> R) :
  enum_chain_increasing chain -> enum_converges chain out ->
  (forall x, 0 <= f x) ->
  countable_upper (fun n => expect f (chain n)) = expect f out.
Proof.
  exact (@enum_monotone_converges_upper_eqtype
    (@Equality.Pack (EnumCouplingClassical.carrier A)
      (Equality.on (EnumCouplingClassical.carrier A))) chain out f).
Qed.

Lemma enum_real_expect_indicator {A} (mu : Enum A) (P : A -> bool) :
  expect (fun x => if P x then 1 else 0) mu =
  ratr (enum_expect (fun x => if P x then 1 else 0) mu).
Proof.
...
Proof.
  ...
  - reflexivity.
  - change (expect (fun x => upper (k x) (fun y => f (obs y))) (subenum_raw node) =
      expect f (bind_Enum (subenum_raw node) (fun x => subenum_raw (front x)))).
  ...
  - cbn [free_omega_upper].
    have Hrows : (fun n => upper (chain n) (fun x => f (obs x))) =
      (fun n => expect f (subenum_raw (outs n))).
    { apply functional_extensionality=> n. exact (IH n f Hf). }
    rewrite Hrows. apply enum_monotone_converges_upper; [|exact Hlim|].
    + intros P n m Hnm.
      have HP : forall y, 0 <= (if P y then (1 : R) else 0) /\
          (if P y then (1 : R) else 0) <= 1.
      { intro y. destruct (P y); split; try exact: ler01; exact: lexx. }
      have Hstep : forall i,
          expect (fun y => if P y then 1 else 0) (subenum_raw (outs i)) <=
          expect (fun y => if P y then 1 else 0) (subenum_raw (outs (S i))).
...
Theorem free_omega_denotes_upper {A O} (obs : A -> O)
    (mu : FreeOmega SubEnum A) (out : SubEnum O) (f : O -> R) :
...
Proof.
  ...
  apply/eqP. rewrite eq_le. apply/andP. split.
  - eapply subenum_lift_real_expect; [exact Heq|]. intros x y ->. exact: lexx.
  - eapply subenum_lift_real_expect; [apply sem_lift_sym; exact Heq|].
