...
Require Import PTree.Prob.Backend.Common.RatSubTypes PTree.Prob.Backend.Enum.Representation PTree.Prob.Backend.Enum.Map PTree.Prob.Backend.Enum.Iteration PTree.Prob.Backend.Enum.Measure.
...
(** Internal scalar model for the raw weighted Enum backend.  Unlike the
    SubEnum model, neither total mass nor intermediate upper expectations
...
Fixpoint enum_extended_expect {A} (f : A -> \bar R) (mu : Enum A) : \bar R :=
  match mu with
  | nil => 0
  | (p, x) :: tail => (ratr (Qval p))%:E * f x + enum_extended_expect f tail
  end.

Lemma enum_extended_expect_nonnegative {A} (f : A -> \bar R) mu :
  (forall x, 0 <= f x) -> 0 <= enum_extended_expect f mu.
Proof.
...
Lemma enum_extended_expect_mono {A} (f g : A -> \bar R) mu :
  (forall x, f x <= g x) -> enum_extended_expect f mu <= enum_extended_expect g mu.
Proof.
...
Lemma enum_extended_expect_zero {A} (mu : Enum A) :
  enum_extended_expect (fun _ => 0) mu = 0.
Proof. by elim: mu=> [|[p x] tail IH] //=; rewrite mule0 IH adde0. Qed.

Lemma enum_extended_expect_app {A} (f : A -> \bar R) mu nu :
  enum_extended_expect f (mu ++ nu) = enum_extended_expect f mu + enum_extended_expect f nu.
Proof. by elim: mu=> [|[p x] tail IH] /=; rewrite ?add0e ?IH ?addeA. Qed.

Lemma enum_extended_expect_scale {A} (f : A -> \bar R) p mu :
  (forall x, 0 <= f x) ->
  enum_extended_expect f (scale_Enum p mu) =
    (ratr (Qval p))%:E * enum_extended_expect f mu.
Proof.
  ...
  - exact: enum_extended_expect_nonnegative.
  congr (_ + _).
...
Lemma enum_extended_expect_bind {A B} (f : B -> \bar R)
    (mu : Enum A) (k : A -> Enum B) :
  (forall x, 0 <= f x) ->
  enum_extended_expect f (bind_Enum mu k) =
    enum_extended_expect (fun x => enum_extended_expect f (k x)) mu.
Proof.
  move=> Hf. elim: mu=> [|[p x] tail IH] //=.
  by rewrite enum_extended_expect_app enum_extended_expect_scale // IH.
Qed.

Lemma enum_extended_expect_rat {A} (f : A -> rat) mu :
  enum_extended_expect (fun x => (ratr (f x))%:E) mu = (ratr (enum_expect f mu))%:E.
Proof.
  elim: mu=> [|[p x] tail IH] /=; first by rewrite rmorph0.
  by rewrite IH rmorphD rmorphM EFinD EFinM.
Qed.

Lemma enum_extended_expect_one {A} (mu : Enum A) :
  enum_extended_expect (fun _ => 1) mu = (ratr (enum_expect (fun _ => 1) mu))%:E.
Proof.
  rewrite (_ : (fun _ : A => (1 : \bar R)) = (fun _ => (ratr (1 : rat))%:E));
    last by apply functional_extensionality=> x; rewrite rmorph1.
  exact: enum_extended_expect_rat.
Qed.
...
Fixpoint free_omega_extended_upper {A} (mu : FreeOmega Enum A)
    (f : A -> \bar R) : \bar R :=
  match mu with
  | FORet x => f x
  | FOZero => 0
  | @FOSample _ _ X node k =>
      enum_extended_expect (fun x => free_omega_extended_upper (k x) f) node
  | FOLub c => extended_upper (fun n => free_omega_extended_upper (c n) f)
  end.

Lemma free_omega_extended_upper_nonnegative {A} (mu : FreeOmega Enum A)
    (f : A -> \bar R) :
...
Lemma free_omega_extended_upper_mono {A} (mu : FreeOmega Enum A)
    (f g : A -> \bar R) :
  (forall x, f x <= g x) ->
...
Proof.
  ...
  - exact: Hfg.
  - exact: lexx.
  - exact: enum_extended_expect_mono.
  - exact: extended_upper_mono.
Qed.

Lemma free_omega_extended_upper_bind {A B} (mu : FreeOmega Enum A)
    (k : A -> FreeOmega Enum B) (f : B -> \bar R) :
