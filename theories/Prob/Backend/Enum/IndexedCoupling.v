...
Require Import PTree.Prob.Backend.Common.RatSubTypes PTree.Prob.Backend.Enum.Representation PTree.Prob.Backend.Enum.Bind PTree.Prob.Backend.Enum.Coupling.
...
Lemma index_from_in_ge {A} n (mu : Enum A) p i :
...
Lemma index_from_bind_as_position_bind {A B}
    (mu : Enum A) (k : A -> Enum B) start offset :
  index_from offset (bind_Enum mu k) =
  bind_Enum (index_from start mu)
    (indexed_bind_block_from mu k start offset).
Proof.
  revert start offset.
  induction mu as [|[p a] mu IH]; intros start offset; cbn=> //.
  rewrite index_from_app index_from_scale size_scale_Enum.
  rewrite (IH start.+1 (Nat.add offset (size (k a)))).
  congr (_ ++ _).
  - cbn. by rewrite Nat.eqb_refl.
  - apply bind_Enum_ext_in=> q i Hi. cbn.
