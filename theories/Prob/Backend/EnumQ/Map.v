...
Definition emap {A B} (f : A -> B) (mu : EnumQ A) : EnumQ B :=
  map (fun px => (fst px, f (snd px))) mu.

Lemma emap_id {A} (mu : EnumQ A) :
  emap id mu = mu.
Proof.
  elim: mu => [//|[p a] mu IH] //=.
  by rewrite IH.
...
Lemma emap_scale {A B} (f : A -> B) p (mu : EnumQ A) :
  emap f (scale_EnumQ p mu) = scale_EnumQ p (emap f mu).
...
Lemma emap_bind {A B C} (f : B -> C)
  ...
  emap f (bind_EnumQ mu k) =
  bind_EnumQ mu (fun x => emap f (k x)).
Proof.
  elim: mu=> [|[p a] mu IH] //=.
...
Lemma bind_ret_emap {A B} (f : A -> B) (mu : EnumQ A) :
  bind_EnumQ mu (fun x => ret_EnumQ (f x)) = emap f mu.
Proof.
  elim: mu=> [|[p a] mu IH] //=.
  by rewrite mulr1 IH.
Qed.
...
Lemma acc_mass_filter {A : eqType} (P : pred A) (mu : EnumQ A) (a : A) :
...
Lemma enumQ_filter_proper {A : eqType} (P : pred A) :
  Proper (EqEnumQ ==> EqEnumQ)
    (fun mu : EnumQ A => [seq px <- mu | P (snd px)]).
Proof.
  move=> mu nu H a.
...
Proof.
  move=> mu nu H b.
  set P := fun x : A => f x == b.
  have HF : [seq px <- mu | P (snd px)] ==EnumQ
            [seq px <- nu | P (snd px)].
    exact: enumQ_filter_proper H.
