...
From PTree.Prob Require Import RatSubTypes DiscreteMC FrontierLiftEnum TwoLevelMeasureSubEnum
...
Theorem free_omega_upper_continuous {A} (mu : FreeOmega SubEnum A)
    (tests : nat -> A -> R) :
  (forall n x, 0 <= tests n x /\ tests n x <= 1) ->
  (forall n x, tests n x <= tests (S n) x) ->
...
Proof.
  ...
  - have Hrows : (fun x => upper (k x) (fun y => countable_upper (fun n => tests n y))) =
    ...
    rewrite Hrows. apply enum_real_expect_countable.
...
Theorem free_omega_sample_lub_upper {A X} (mu : SubEnum X)
    (chain : X -> nat -> FreeOmega SubEnum A) (f : A -> R) :
  enum_ae (subenum_raw mu)
...
Proof.
  intros Hi Hf. cbn [free_omega_upper]. apply enum_real_expect_countable_ae.
...
Theorem free_omega_bind_lub_upper {A X}
    (source : nat -> FreeOmega SubEnum X)
    (kernels : X -> nat -> FreeOmega SubEnum A) (f : A -> R) :
