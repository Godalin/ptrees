(** Role: One-way external validation of SubEnum PTree stable hitting.
...
From PTree.Prob.Backend.SubEnum Require Import Measure Domain Expectation.
...
Variable R : realType.
Context {S H : Type}.
Variable K : S -> FreeOmega SubEnum (stable_target S H).
...
Local Notation state := (ptree' E SubEnum A).
Local Notation head := (stable_head E SubEnum A).
Local Notation K := (@ptree_primitive_kernel E SubEnum (FreeOmega SubEnum)
  FI FreeOmegaMixedMeasure A).
Local Notation approx := (@ptree_hitting_approx E SubEnum (FreeOmega SubEnum)
  FI FreeOmegaMixedMeasure FO A).
Local Notation hits := (@ptree_stable_hitting E SubEnum (FreeOmega SubEnum)
  FI FreeOmegaMixedMeasure FO A).
...
Definition ptree_domain_kernel (s : state) : OmegaVal R (stable_target state head) :=
  ...
  | ProbF _ mu k => oval_bind (subenum_domain R mu)
...
Lemma ptree_domain_approx_prob_zero {X} (mu : SubEnum X) k f :
  oval_eval (ptree_domain_approx O (ProbF mu k)) f = 0.
Proof. apply enum_real_expect_zero. Qed.

Lemma ptree_domain_approx_prob_succ {X} n (mu : SubEnum X) k f :
  oval_eval (ptree_domain_approx (Datatypes.S n) (ProbF mu k)) f =
  enum_real_expect (fun x => oval_eval (ptree_domain_approx n (observe (k x))) f)
    (subenum_raw mu).
Proof. reflexivity. Qed.
