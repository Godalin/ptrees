...
From PTree.Eq.FreeOmega Require Import FiniteInternal.
...
Lemma finite_internal_advance_lub
...
Lemma finite_internal_cut_bind_lub t
    (next : ptree E MN R -> MF (stable_head E MN R))
    (kernels : ptree E MN R -> nat -> MF (stable_head E MN R)) :
  (forall u, free_omega_qlift eq (next u) (FOLub (kernels u))) ->
  (forall u n, free_omega_approx eq (kernels u n) (kernels u (S n))) ->
  free_omega_qlift eq (free_omega_bind (cut t) next)
...
Proof.
  ...
  - apply free_omega_support_lift_bind_diagonal.
    + exact (proj1 (trunc_spec t)).
    + exact Hinc.
    + apply free_omega_qlift_support. exact (proj1 (proj2 (trunc_spec t))).
    + intro u. apply free_omega_qlift_support. apply Hnext.
Qed.
...
Proof.
  induction n as [|n IH] in t |- *;
    unfold finite_internal_grid at 1; cbn [finite_internal_rounds].
  - apply finite_internal_cut_bind_lub.
    + intro u. apply FOQLLubConstantR.
