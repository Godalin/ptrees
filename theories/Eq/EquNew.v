...
Variant equb (eq_ : ptree E M R1 -> ptree E M R2 -> Prop)
  ...
  | Eq_Prob {X : eqType} {μ1 μ2 : M X} k1 k2
...
Lemma equ_prob_invT {X Y : eqType} {S}
...
Lemma equ_prob_invE {X : eqType} {S}
...
Lemma equb_prob_invT {X Y : eqType} {S}
    (μ1 : M X) (μ2 : M Y)
...
Lemma equb_prob_invE {X : eqType} {S} (μ1 μ2 : M X)
    (k1 : _ -> ptree E M S) k2
...
#[global] Instance equ_ProbF {X : eqType}
  : Proper (@disc_RT M _ X X eq ==> pointwise_relation _ (equ eq) ==> going (equ eq))
    (@ProbF E M R _ _).
...
Proof.
  ...
  - dependent destruction H5; subst. dependent destruction H6.
...
Proof. apply tower.
  2:{ intros RC IH y1 y2 Hb.
      ...
      - rewrite <- H1. rewrite <- H2. econstructor.
        ...
        intros x11 x22. intros HS12. apply (b_chain RC).
        apply Hk. auto.
      - rewrite <- H1. rewrite <- H2. econstructor; auto.
        intro x. apply IH. cbn. repeat econstructor; auto.
        intros x11 x22. intros HS12. apply (b_chain RC).
        apply Hk. auto.
    }
...
Notation bind_ t k :=
  ...
  | RetF r => k%function r
  | TauF u => Tau (bind u k)
  | VisF _ e ke => Vis e (fun x => bind (ke x) k)
  | ProbF _ μ kμ => Prob μ (fun x => bind (kμ x) k)
  end.
...
Lemma bind_prob {X : eqType} {Y Z} (μ : M X)
    (k : X -> ptree E M Y) (g : Y -> ptree E M Z)
...
Lemma prob_equ_bind {X Y} {Z : eqType}
...
Proof.
  ...
  - rewrite (ptree_eta t), Heqp, bind_tau in H0. step in H0. inversion H0.
  - rewrite (ptree_eta t), Heqp, bind_vis in H0. step in H0. inversion H0.
  - rewrite (ptree_eta t), Heqp, bind_prob in H0.
    apply equ_prob_invT in H0 as ?. subst.
    step in H0. dependent destruction H0.
    right. exists k0. split.
