...
From PTree.Interp Require Import Kernel Preservation.
...
Proof.
  ...
  assert (Hlift : @sem_lift MF FI _ _ eq mu out).
...
Proof.
  ...
  - apply (proj2 (ptree_stable_hitting_tau_iff (FI := FI) (FO := FO) _ _)).
    eapply (ptree_stable_hitting_bind (FI := FI) (FO := FO)
      (MX := MX)); [apply (Preservation.bind_cofinal_all (BindOrd := BindOrd) (MixedOrd := MixedOrd) (Directed := Directed))|exact Hmu|exact Hfront2].
