...
consumed by existing ReaderT/WriterT/ExceptT law proofs. The full Eq1-wide
`iteration_uniform` package is now proved by a direct restart machine that
does not encode loop state as an auxiliary event response. Actual State and
Exception fold squares instantiate it with a PTree target; ReaderT/WriterT/
ExceptT inherit it. The old protocol-specific universe rejection remains
tested, but no longer blocks these clients. Core syntax, upstream interfaces
and checker boundaries are unchanged. A Tau-free behavioral fixed point and
state-dependent finite Tau stuttering were already proved. See
[direct iteration uniformity](docs/DIRECT_ITERATION.md) and
[iteration algebra](docs/ITERATION_ALGEBRA.md).
